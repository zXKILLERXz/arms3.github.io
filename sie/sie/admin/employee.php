              ...
              <div class="box">
                ...
                <div class="box-body">
                  <table id="example1" class="table table-bordered">
                    ...
                    <tbody>
                      <?php
                        $sql = "SELECT *, employees.id_employee AS empid FROM employees LEFT JOIN position ON position.id = employees.position_id";
      ...
      function getRow(id){
        $.ajax({
          ...
          success: function(response){
            ...
            $('.document').html(response.document);
            $('.del_employee_name').html(response.firstname+' '+response.lastname);
            $('#employee_name').html(response.firstname+' '+response.lastname);
            $('#edit_firstname').val(response.firstname);
            $('#edit_lastname').val(response.lastname);
            $('#datepicker_edit').val(response.birthdate);      
            $('#sex_val').val(response.gender_id).html(response.gender);
            $('#stat_val').val(response.status_id).html(response.status);
            $('#edit_address').val(response.address);
            $('#cities_val').val(response.city).html(response.city);
